import { AddSignInEmailPasswordError, AnonymousUserError, CreateUserError, EmailNeedsConfirmationError, PasswordResetError, RefreshTokensError, SetPasswordResetError, SetValidationCodeError, SignInEmailPasswordError, UpdatePasswordError, ValidationCodeError, } from "https://baseless.dev/w/shared/auth.js";
import { autoid } from "https://baseless.dev/w/shared/autoid.js";
import { SignJWT } from "https://cdnjs.cloudflare.com/ajax/libs/jose/4.3.7/jwt/sign.js";
...
export class AuthController {
    authDescriptor;
    logger = logger("server/AuthController");
    constructor(authDescriptor) {
        this.authDescriptor = authDescriptor;
    }
    _getMessageTemplate(client, type, locale) {
    ...
    async _sendValidationEmailTo(context, locale, to) {
        const code = autoid(40);
        const template = this._getMessageTemplate(context.client, "validation", locale);
        ...
        if (template) {
            ...
            this.logger.error(`Could not find validation template for locale "${locale}". Validation code is "${code}".`);
        }
    }
    async _createJWTs(context, user, scope = "*") {
    ...
    async sendValidationEmail(context, locale, email) {
        try {
            const user = await context.auth.getUserByEmail(email);
            if (!user.email || user.emailConfirmed) {
                throw new SetValidationCodeError();
            }
            await this._sendValidationEmailTo(context, locale, user.email);
            return {};
        }
        catch (err) {
    ...
    async validateEmailWithCode(context, email, code) {
        try {
    ...
    async sendPasswordResetEmail(context, locale, email) {
        try {
            const code = autoid(40);
            const template = this._getMessageTemplate(context.client, "passwordReset", locale);
            ...
            if (template) {
                ...
                this.logger.error(`Could not find password reset template for locale "${locale}". Reset password code is "${code}".`);
            }
            return {};
        }
        catch (err) {
    ...
    async resetPasswordWithCode(context, email, code, password) {
        try {
    ...
    async addSignWithEmailPassword(context, locale, email, password) {
        try {
            const { authDescriptor } = this;
            ...
            await context.auth.addSignInMethodPassword(user.id, await this._hashPassword(password));
            await this._sendValidationEmailTo(context, locale, email);
    ...
    async createUserWithEmail(context, locale, email, password, claimAnonymousId) {
        const { authDescriptor } = this;
        if (!authDescriptor.allowSignMethodPassword) {
    ...
    async signWithEmailPassword(context, email, password) {
        const { authDescriptor } = this;
        if (!authDescriptor.allowSignMethodPassword) {
            this.logger.warn(`Sign in with email and password is not allowed.`);
    ...
    async refreshTokens(context, refresh_token) {
        try {
